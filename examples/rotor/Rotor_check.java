...
public class Rotor_check extends PilotsRuntime {
    private static Logger LOGGER = Logger.getLogger(Rotor_check.class.getName());
    ...
    public Rotor_check(String args[]) {
        super(args);
    ...
    public void produceOutputs() {
        ...
        while (!isEndTime()) {
            ...
            // Error detection
            int mode = -1;
            if (R_LOW<=data.get("e1") && data.get("e1")<=R_HIGH && P_LOW<=data.get("e2") && data.get("e2")<=P_HIGH && Y_LOW<=data.get("e3") && data.get("e3")<=Y_HIGH) {
                mode = 0;	// "Normal"
            } else if (k<R_LOW || k>R_HIGH) {
    ...
    public static void main(String[] args) {
        Rotor_check app = new Rotor_check(args);
        app.startServer();
